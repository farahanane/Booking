            ...
            <div class="container py-5">
                <div class="row justify-content-center py-5">
                    <div class="col-lg-10 pt-lg-5 mt-lg-5 text-center">
                        ...
                        <form action="{{ route('listings.index') }}" method="GET" id="search-form">
                            @csrf
                            <div class="row g-3">
                                <div class="col-md-3">
                    ...
                    <option value="Nabeul" {{ request()->query('location_city') == 'Nabeul' ? 'selected' : '' }}>Nabeul</option>
                                ...
                                <div class="col-md-3">
                                    <input type="date" class="form-control datepicker" name="start_date" value="{{ request()->query('start_date') }}" required>
                                </div>
                                <div class="col-md-3">
                                    <input type="date" class="form-control datepicker" name="end_date" value="{{ request()->query('end_date') }}" required>
                                </div>
                                <div class="col-md-2">
...
<div class="text-end mb-4 position-relative">
    ...
    <div class="filter-dropdown" id="filter-dropdown">
        <form action="{{ route('listings.index') }}" method="GET" id="filter-form">
            <!-- Dynamically preserve all query parameters -->
            @foreach (request()->query() as $key => $value)
                @if ($key !== 'hotel_category' && $key !== 'meal_plan' && $value !== '') <!-- Exclude filter fields and empty values -->
                        ...
                        <div class="package-item" data-listing-id="{{ $listing->id }}">
                            ...
                            <div class="text-center p-4">
                                <div class="d-flex mb-3 justify-content-start">
                                    ...
                                    <h4 class="flex-fill py-2 fw-bold" style="color: #2C3E50;">
                                        {{ number_format($listing->min_price, 2) }}<small><sup>DT</sup></small>
                                    </h4>
                                </div>
                                <div class="mb-3">
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        form.addEventListener('submit', (e) => {
            ...
            const listings = @json($listings); // Pass listings with rooms

            let validDate = false;
            listings.forEach(listing => {
                listing.rooms.forEach(room => {
                    const roomStart = new Date(room.start_date);
                    const roomEnd = new Date(room.end_date);
                    if (startDate >= roomStart && endDate <= roomEnd) {
                        validDate = true;
                    }
                });
            });
            ...
            if (!validDate) {
                e.preventDefault();
                alert('You can\'t book in that date range. Please select a date within available room periods.');
            }
        });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add click event to package-item to redirect to show page
    document.querySelectorAll('.package-item').forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            window.location.href = '{{ route("listings.show", ["listing" => ":id"]) }}'.replace(':id', listingId);
        });
    });
});
</script>
